...
A watch with a [hook](run.md#hooks) that has failed on **3 consecutive** runs (a non-zero exit or a timeout) is reported as `attention` with kind `hook-failing`, and the summary names the event, the command, the failure count, and the last error. Like `snapshots-failing`, it **clears itself** the moment that hook next succeeds — there is nothing to acknowledge. A failing **global** `[hooks]` hook has no watch to attach to, so it is reported on its own row labeled `[hooks]`.
