...
The `attention` state also covers the **secret-quarantine** case, distinguished by its kind `secret-quarantined`: the watch's most recent snapshot pass withheld one or more newly-added files as likely secrets. The summary carries the count and points at the fix (move or delete the file, or exclude it) but **never the file names or their contents**. It **self-clears** with no acknowledgement — quarantine is re-derived every pass, so once the file is moved, deleted, or excluded, the next clean pass returns the watch to `ok` on its own. Scanning is per-watch configurable (`secret_scan`; see [`config`](config.md#secret-scanning-secret_scan--secret_patterns)).
