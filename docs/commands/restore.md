...
The protective snapshot honors [secret quarantine](snapshot.md#secret-quarantine) like any other pass: a newly-added file that looks like a secret is withheld from it. This never loses data — a withheld file is untracked, so the checkout that follows leaves it on disk untouched.
