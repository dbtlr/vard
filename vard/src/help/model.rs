...
pub struct FlagEntry {
    ...
    /// Long flag like `--color`. `None` if the arg has only a short form.
    pub long: Option<String>,
    /// Value placeholder like `<WHEN>`. `None` if the flag takes no value.
...
/// A global option. Globals always render in one block (no collapse) with one
/// short line each.
#[derive(Debug, Clone)]
pub struct GlobalEntry {
    pub short: Option<char>,
    pub long: Option<String>,
    /// `None` if the global takes no value.
    pub value_name: Option<String>,
    /// Constrained to ≤70 chars per the help-output v2 spec §2.2.
    pub short_desc: String,
}

/// Phase-deferred extras: canned examples and conceptual prose sections.
#[derive(Debug, Clone, Default)]
pub struct HelpExtras {
    /// Hand-authored examples: `Vec<(command_str, comment_str)>`.
    pub canned_examples: Vec<(String, String)>,
    /// Conceptual prose sections: `Vec<(heading, body)>`.
...
pub struct HelpModel {
    ...
    pub about: String,
    /// Optional multi-line description (from clap `long_about`). Rendered only
    /// in `--help`.
    pub long_about: Option<String>,
    /// Positional arguments, in declaration order.
    pub positionals: Vec<FlagEntry>,
    /// Flag groups in display order (NOT clap declaration order). The extractor
    /// groups by `help_heading` and orders groups by first-seen.
